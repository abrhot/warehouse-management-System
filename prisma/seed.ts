...
async function main() {
  ...
  const suppliers = await Promise.all(
    suppliersData.map((s) => prisma.supplier.upsert({ where: { name: s.name }, update: {}, create: { name: s.name } }))
  );
  ...
  const categories = await Promise.all(
    categoriesData.map((c) => prisma.category.upsert({ where: { name: c.name }, update: {}, create: { name: c.name } }))
  );
