...
function DashboardPage() {
    ...
    return (<div className="flex flex-col min-h-screen bg-[#fafbf8] text-[#141b0e] font-['Manrope','Noto_Sans',sans-serif]">
      {/* Header */}
      <header className="flex items-center justify-between border-b border-[#edf3e8] px-10 py-3">
        ...
        <div className="flex items-center gap-4">
          <button onClick={handleLogout} className="rounded-lg bg-[#78df24] px-4 py-2 font-bold text-sm">
            Logout
          </button>
          ...
          <div className="w-10 h-10 rounded-full bg-cover bg-center" style={{ backgroundImage: `url("/images/supervisor.jpg")` }}/>
        </div>
      </header>
      ...
      <main className="flex flex-col items-center px-10 py-5">
        <div className="w-full max-w-[960px]">
        ...
        ].map(([label, value, change, color]) => (<div key={label} className="border border-[#dae6d1] rounded-lg p-4 flex flex-col gap-1">
        ...
        ].map(([time, product, qty, user]) => (<tr key={time} className="border-t border-[#dae6d1] text-sm">
          ...
          <div className="overflow-x-auto px-4">
            ...
            ["Network Switch", "8"],
            ["Power Supply Unit", "5"],
        ].map(([product, qty]) => (<tr key={product} className="border-t border-[#dae6d1] text-sm">
                    <td className="px-4 py-2">{product}</td>
                    <td className="px-4 py-2 text-[#6f9550]">{qty}</td>
