...
const StockInForm = () => {
    ...
    const [newProductName, setNewProductName] = (0, react_1.useState)('');
    const [newProductCategory, setNewProductCategory] = (0, react_1.useState)('');
    ...
    return (<div className="mx-auto w-full max-w-2xl rounded-xl bg-white p-8 shadow-lg">
      ...
      <div className="mb-6 grid grid-cols-2 gap-2 rounded-lg bg-gray-100 p-1">
        <button onClick={() => setIsCreatingNew(false)} className={`rounded-md px-4 py-2 text-sm font-semibold transition-colors duration-200 ${!isCreatingNew ? 'bg-white shadow text-gray-800' : 'bg-transparent text-gray-500 hover:bg-gray-200'}`}>
          Existing Product
        </button>
        <button onClick={() => setIsCreatingNew(true)} className={`rounded-md px-4 py-2 text-sm font-semibold transition-colors duration-200 ${isCreatingNew ? 'bg-white shadow text-gray-800' : 'bg-transparent text-gray-500 hover:bg-gray-200'}`}>
          New Product
        </button>
      </div>

      <div className="space-y-6">
        {isCreatingNew ? (<div className="space-y-6">
            <div>
        ...
        <div className="flex justify-end pt-4">
          <button onClick={handleAddStock} className="flex items-center justify-center rounded-lg bg-green-400 px-5 py-2 text-sm font-bold text-white shadow-md transition-colors hover:bg-green-500 focus:outline-none focus:ring-2 focus:ring-green-400 focus:ring-offset-2">
            ➕ Request Stock In
          </button>
        </div>
      </div>
...
exports.StockInForm = StockInForm;
