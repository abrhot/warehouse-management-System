...
const ProductTable = ({ products, searchTerm, onSearchChange, isLoading, error, }) => {
      ...
      <div className="flex flex-wrap justify-between items-center gap-4 mb-4">
        <div className="flex items-center gap-3">
          <div className="relative">
            <input type="text" placeholder="Search by name or ID" className="rounded-xl h-10 pl-10 pr-4 text-sm text-[#141b0e] placeholder-[#6f9550] bg-[#edf3e8] focus:outline-none" value={searchTerm} onChange={(e) => onSearchChange(e.target.value)}/>
            <div className="absolute top-2.5 left-3 text-[#6f9550]">🔍</div>
          </div>

          <link_1.default href="/stock/in">
        ...
        <table className="w-full min-w-[800px]">
          ...
          <tbody>
            {products.length === 0 ? (<tr>
              ...
              </tr>) : (products.map(product => (<tr key={product.id} className="border-t border-t-[#dae6d1]">
