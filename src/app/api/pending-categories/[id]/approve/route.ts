import { NextRequest, NextResponse } from 'next/server';
import prisma from '@/lib/prisma';
import { z } from 'zod';

const approveSchema = z.object({
  notes: z.string().optional(),
});

export async function POST(
  request: NextRequest,
  { params }: { params: { id: string } }
) {
  try {
    const userRole = request.headers.get('x-user-role');
    const userId = request.headers.get('x-user-id');
    
    if (userRole !== 'ADMIN' || !userId) {
      return NextResponse.json({ error: 'Unauthorized' }, { status: 403 });
    }

    const body = await request.json();
    const { notes } = approveSchema.parse(body);

    // Find the pending category
    const pendingCategory = await prisma.pendingCategory.findUnique({
      where: { id: params.id },
    });

    if (!pendingCategory) {
      return NextResponse.json(
        { error: 'Pending category not found' },
        { status: 404 }
      );
    }

    if (pendingCategory.status !== 'PENDING') {
      return NextResponse.json(
        { error: 'Category has already been reviewed' },
        { status: 400 }
      );
    }

    // Create the actual category and update pending status in a transaction
    const result = await prisma.$transaction(async (tx) => {
      // Create the actual category
      const category = await tx.category.create({
        data: {
          name: pendingCategory.name,
          description: pendingCategory.description,
        },
      });

      // Update pending category status
      const updatedPending = await tx.pendingCategory.update({
        where: { id: params.id },
        data: {
          status: 'APPROVED',
          reviewedBy: userId,
          notes,
        },
      });

      return { category, updatedPending };
    });

    return NextResponse.json({
      message: 'Category approved successfully',
      categoryId: result.category.id,
    });
  } catch (error) {
    if (error instanceof z.ZodError) {
      return NextResponse.json(
        { error: 'Validation failed', details: error.errors },
        { status: 400 }
      );
    }

    console.error('Error approving category:', error);
    return NextResponse.json(
      { error: 'Failed to approve category' },
      { status: 500 }
    );
  }
}
