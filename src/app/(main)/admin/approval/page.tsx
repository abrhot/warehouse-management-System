...
const Notification = ({ message, type, onDismiss }: { message: string; type: 'success' | 'error', onDismiss: () => void }) => {
  ...
  const baseClasses = 'fixed top-5 right-5 p-4 rounded-md shadow-lg text-white text-sm z-50';
...
export default function ApprovalPage() {
  ...
  const fetchRequests = async () => {
    try {
      ...
      setNotification({ message: "Failed to load pending requests.", type: 'error' });
    }
  }
  ...
  const handleAction = async (id: string, approved: boolean) => {
    ...
    try {
      ...
      if (res.ok) {
        setNotification({ message: approved ? "Request approved successfully!" : "Request rejected.", type: 'success' });
        fetchRequests(); // Refresh the list
      } else {
        const err = await res.json();
        setNotification({ message: `Error: ${err.error || "An unknown error occurred"}`, type: 'error' });
      }
    } catch (error) {
      setNotification({ message: "An unexpected network error occurred.", type: 'error' });
    } finally {
