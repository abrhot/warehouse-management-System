  ...
  return (
    <Card className="p-4 border border-gray-200 shadow-sm bg-white">
      <CardHeader className="p-0 mb-4">
        <CardTitle className="text-xl font-bold text-blue-700">Products by Category</CardTitle>
      </CardHeader>
      <CardContent className="p-0">
        <Accordion type="single" collapsible className="w-full">
          {categories.map((category) => (
            <AccordionItem key={category.id} value={category.name} className="border-b last:border-b-0">
              <AccordionTrigger className="hover:bg-blue-50 px-4 py-3 rounded-md transition-colors duration-150">
